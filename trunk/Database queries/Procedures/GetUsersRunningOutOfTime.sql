...
as 
begin
select us.UserId, FirstName, Surname, isnull(PhoneNumber, 'Not available') as PhoneNumber, case when deadline < GETDATE() then 'OutOfTime' when DATEDIFF(MINUTE, GETDATE(), deadline) <=60 and hourWarning=0 then 'hour' when DATEDIFF(MINUTE, GETDATE(), deadline) <=5 and [5minutesWarning]=0 then '5minutes' end as warningType 
...
update Users
set hourWarning = case when warningType='hour' then 1 else hourWarning end,
